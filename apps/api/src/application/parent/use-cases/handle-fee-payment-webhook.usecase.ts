...
export class HandleFeePaymentWebhookUseCase {
  ...
  async execute(
    ...
    // Amount cross-validation: Cashfree order_amount must match stored totalAmount
    const webhookAmount = payload.data?.order?.order_amount;
    if (webhookAmount !== undefined && webhookAmount !== payment.totalAmount) {
      this.logger.error('Fee payment webhook amount mismatch', {
        orderId,
        webhookAmount,
        storedAmount: payment.totalAmount,
      });
